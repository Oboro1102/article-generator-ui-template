...
const goToNewGenerate = () => {
  articleStore.goToNewGenerate()
...
<template>
  <header
    class="animate sticky top-0 left-0 z-50 flex w-full items-center justify-between gap-4 bg-white px-4 py-2.5"
  >
    <router-link
      to="/"
      class="animate text-xl font-bold text-black hover:animate-[zoom_1s_ease-in-out]"
      >文章生成系統模板</router-link
    >
    <Button
      size="small"
      label="建立新文章"
      ...
      @click="goToNewGenerate"
    />
    <Button
      size="small"
      severity="secondary"
      text
      v-tooltip.bottom="'系統選單'"
    ...
    <TieredMenu
      ...
      :pt="{
        ...
        itemContent: { class: 'text-(--p-button-text-secondary-color)!' },
      }"
    />
    <Dialog
